...
class StatusesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create status" do
    assert_difference('Status.count') do
      post statuses_url, params: { status: { contenido: @status.contenido, name: @status.name } }
    end
  ...
  test "should update status" do
    patch status_url(@status), params: { status: { contenido: @status.contenido, name: @status.name } }
